...
var vm = new Vue({
	...
	methods: {
    	...
    	save: function (isSubmit){
            ...
            $.ajax({
					...
					type: "POST",
					url: httpurl+'government/'+this.msg,
			    	data: {
		...
		autoAdd:function () {
			$.ajax({
				...
				type: "POST",
				url: httpurl+'government/autoAdd',
				data: {
					tm:new Date().getTime()
					},
				dataType:"json",
		...
		isAllpull:function () {
			...
			//如果是可以选填  可以不进行填报  
			//但是如果是选填的 已经填报 需要判断其状态   必须是已经正式提交状态
		...
		getInfo:function(){
			$.ajax({
				...
				type: "POST",
				url: httpurl+'government/getInfo',
				data: {
					tm:new Date().getTime()
					},
				dataType:"json",
		...
		//点击链接弹出表单填写页面
		toFulldata:function(e,allName,form_id,GOVERNMENT_ID,allUrl,APPROVEFORM_ID){   //e this 对象  allName：该表单的名字+html  form_id:表单的id  GOVERNMENT_ID_ID:审批id
			...
			if(fulldata_id==''||fulldata_id==undefined){
				url=allUrl+"?FID="+form_id+"&AFT="+vm.appType2+"&AID="+GOVERNMENT_ID;
			}else{
				url=allUrl+"?FID="+form_id+"&AFT="+vm.appType2+"&AID="+GOVERNMENT_ID+"&FDID="+fulldata_id+"&AFID="+APPROVEFORM_ID;
			}
			...
			diag.CancelEvent = function(){ //关闭事件
				var varSon = diag.innerFrame.contentWindow.document.getElementById('showform');
				 ...
				 if(SUB_STATUS_val!=''&&SUB_STATUS_val!=null&&SUB_STATUS_val!=undefined){  //状态为暂存
					...
					if(SUB_STATUS_val==1&&formdata_id!=''&&formdata_id!=null&&formdata_id!=undefined){
						$(e.target).next().next().css("display","inline");
						$(e.target).next().css("display","none");
					}else if(SUB_STATUS_val==0&&formdata_id!=''&&formdata_id!=null&&formdata_id!=undefined){
    ...
    layui.use('upload', function() {
        ...
        var demoListView = demoListView1=$('#demoList')
            ...
            url: httpurl+'/government/attachmentLoad',
            elem: '#testList'
            ,accept: 'file',
			data: {
				P_TYPE: '2',   //表示为行政审批
        ...
        function deleteFile(id) {
            $("#demoList").on('click', '.demo-delete', function () {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: httpurl+'government/deleteAttachment',
                    data: {
                        ID:id,
                    },
                    success: function(data){
