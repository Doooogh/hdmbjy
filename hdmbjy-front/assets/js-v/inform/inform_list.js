			...
			$.ajax({
				...
				success: function(data){
					if("sucecss" == data.result){
						vm.getList();
						top.vm.tabMethod(); //websocket即时通讯用于给待办人发送新任务消息 ，fhtaskmsg()函数 在 WebRoot\assets\js-v\index.js
					}else if ("exception" == data.result){
			     	showException("通知查询不到",data.exception);		//显示异常
			     }
				}
			})
