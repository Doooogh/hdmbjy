...
var vm = new Vue({
	...
	methods: {
        ...
        init() {
			...
			var TP=this.getUrlKey('tPage');   //教师类型
			var IDIS=this.getUrlKey('IDIS');   //教师类型
			if(null!=OID&&''!=OID){
        ...
        getList: function(){
        	...
        	$.ajax({
        		...
        		data: {
					ORGANIZATION_ID:this.ORGANIZATION_ID,  //机构 id
    	...
    	goAdd: function (){
			...
			/* $.ajax({
						...
						type: "POST",
						async:false,
						url: httpurl+'scuser/checkAdd',  //一个机构只能新增一个机构负责人
				    	data: {
				    	tm:new Date().getTime(),
						ORGANIZATION_ID:vm.ORGANIZATION_ID,  //机构id
						},
						dataType:"json",
						success: function(data){
				            if("success" == data.result){
								if(!data.isOK){
									swal("", "机构只能拥有一个负责人!", "error");
										return false;
								}else{
									var diag = new top.Dialog();
									diag.Drag=true;
				            	...
				            	showException("机构负责人管理",data.exception);//显示异常
				            	$("#showform").show();
				        		$("#jiazai").hide();
				            }
				        }
        	...
        	$.ajax({
        		...
        		success: function(data){
        		 if("success" == data.result){
        			...
        			vm.fromExcel = data.fromExcel;						//导入到excel权限
        		 }else if ("exception" == data.result){
		 ...
		 * 机构用户详细资料
		 */
		goDetail:function(SCUSER_ID,ORGANIZATION_ID,TYPE){
			window.location.href="scuser_detail.html?scuId="+SCUSER_ID+"&ORGANIZATION_ID="+ORGANIZATION_ID+"&type="+TYPE;
		},
		/**
		 * 机构用户完善资料
		 */
