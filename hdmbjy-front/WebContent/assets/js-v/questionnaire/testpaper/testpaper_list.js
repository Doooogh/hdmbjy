var vm = new Vue({
	...
	methods: {
    	...
    	goAdd: function (){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="新增";
    		 diag.URL = '../../questionnaire/testpaper/testpaper_edit.html';
    		 diag.Width = 1000;
    		 diag.Height = 800;
    	...
    	goEdit: function(id){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="编辑";
    		 diag.URL = '../../questionnaire/testpaper/testpaper_edit.html?FID='+id;
    		 diag.Width = 1000;
    		 diag.Height = 800;
    		 diag.Modal = true;				//有无遮罩窗口
    		 diag. ShowMaxButton = true;	//最大化按钮
    	...
    	makeAll: function (msg){
    				...
    				if(str==''){
    					...
    					if(msg == '确定要删除选中的数据吗?'){
    						...
    						$.ajax({
    							...
    							type: "POST",
    							url: httpurl+'testpaper/deleteAll?tm='+new Date().getTime(),
    					    	data: {DATA_IDS:str},
    							dataType:'json',
    							success: function(data){
    								if("success" == data.result){
      	...
      	//判断按钮权限，用于是否显示按钮
        hasButton: function(){
        	var keys = 'testpaper:add,testpaper:del,testpaper:edit,toExcel';
        	$.ajax({
        		xhrFields: {
                    withCredentials: true
                },
        		...
        		success: function(data){
        		 if("success" == data.result){
        			vm.add = data.testpaperfhadminadd;		//新增权限
		...
		goExcel: function (){
			swal({
                ...
                if (willDelete) {
                	window.location.href = httpurl+'testpaper/excel';            	
                }
            });
		},
