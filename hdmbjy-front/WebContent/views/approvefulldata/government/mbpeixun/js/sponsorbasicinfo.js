...
var vm = new Vue({
	...
	methods: {
    	...
    	save: function (status){
			...
			if(this.pd.PROVIDE_OTHER_SERVICES == '' || this.pd.PROVIDE_OTHER_SERVICES == undefined){
			        $("#PROVIDE_OTHER_SERVICES").tips({
			            side:3,
			            msg:'请输入举办单位提供其他服务',
			            bg:'#AE81FF',
			            time:2
			        });
			        this.pd.PROVIDE_OTHER_SERVICES = '';
			        this.$refs.PROVIDE_OTHER_SERVICES.focus();
			    return false;
			    }	
		...
		/* 	var result=vm.verifyDate(this.pd.OPINION_DATE);	
            ...
            $.ajax({
			    	...
			    	data: {
						...
						PROVIDE_OTHER_SERVICES:this.pd.PROVIDE_OTHER_SERVICES, 
