...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	_checkFittingToReferencedElem: function() {
		try {
			...
			//if (/Firefox[\/\s](\d+\.\d+)/.demo(navigator.userAgent) && new Number(RegExp.$1)>=3) {
				var newtspans = [];
...
//demo string: abcdefghijklmnopqrstuvwxyz????????????????,.-#+ 1234567890?????ABCDEFGHIJKLMNOPQRSTUVWXYZ;:_'*???????????????!"????$%&/()=?[]{}|<>'~????`\^?????????@?????????????????
...
ORYX.Core.StencilSet.Rules = {
	...
	outgoingEdgeStencils: function(args) {
		...
		// demo each edge, if it can connect to source
