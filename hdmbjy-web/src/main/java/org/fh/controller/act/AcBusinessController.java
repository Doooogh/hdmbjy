...
public class AcBusinessController extends BaseController {
   ...
   private List<String> getExecutedFlows(BpmnModel bpmnModel, List<HistoricActivityInstance> historicActivityInstances) {
	   ...
	   for (int i = 0; i < finishedActivityInstanceList.size(); i++) {
		   HistoricActivityInstance currentActivityInstance=finishedActivityInstanceList.get(i);
            ...
            if(BpmsActivityTypeEnum.PARALLEL_GATEWAY.getType().equals(currentActivityInstance.getActivityType())
                ...
                for(SequenceFlow sequenceFlow : sequenceFlowList) {	 //遍历历史活动节点，找到匹配Flow目标节点的
					HistoricActivityInstance nextHistoricActivityInstance=null;
					for (int i1 = 0; i1 < historicActivityInstances.size(); i1++) {
						HistoricActivityInstance historicActivityInstance=historicActivityInstances.get(i);
						if(currentActivityInstance.getActivityId().equals(historicActivityInstance.getActivityId())){
							nextHistoricActivityInstance=historicActivityInstances.get(i+1);
							break;
						}
					}
					if(nextHistoricActivityInstance.getActivityId().equals(sequenceFlow.getTargetRef())){
						tempMapList.add(UtilMisc.toMap("flowId", sequenceFlow.getId(), "activityStartTime", String.valueOf(nextHistoricActivityInstance.getStartTime().getTime())));

					}
					/*for(HistoricActivityInstance historicActivityInstance : historicActivityInstances){
						if (historicActivityInstance.getStartTime().equals(currentActivityInstance.getEndTime())&&historicActivityInstance.getActivityId().equals(sequenceFlow.getTargetRef())){
